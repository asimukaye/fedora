...
def convert_param_dict_to_ndarray(params: fT.ActorParams_t) -> list[np.ndarray]:
    return [val.cpu().numpy() for val in params.values()]

def convert_param_list_to_ndarray(params: fT.ActorParamlist_t) -> list[np.ndarray]:
    return [val.cpu().numpy() for val in params]

def convert_ndarrays_to_param_lists(nda :list[np.ndarray]) -> fT.ActorParamlist_t:
