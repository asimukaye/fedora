...
def compute_cgsv(
    server_delta: fT.ActorDeltas_t, local_delta: fT.ActorDeltas_t
) -> float:
...
class CgsvStrategy(ABCStrategy):
    ...
    def __init__(self, model: Module, cfg: CgsvCfgProtocol, res_man: ResultManager):
        ...
        if self.cfg.delta_normalize:
            self._update_fn = partial(
                compute_server_delta_w_normalize, gamma=self.cfg.gamma
            )
    ...
    def send_strategy(self, ids: fT.ClientIds_t) -> fT.ClientIns_t:
        ...
        for cid in ids:
            clients_ins[cid] = fT.ClientIns(
                params=self._client_params[cid], metadata={}
            )
        return clients_ins
    ...
    @classmethod
    def client_send_strategy(cls, ins: CgsvIns, result: fT.Result) -> fT.ClientResult:
    ...
    def _sparsify_gradients(
        self, wts: fT.ScalarWeights_t, server_deltas: fT.ActorDeltas_t, beta: float
    ) -> tuple[dict[str, fT.ActorDeltas_t], dict[str, int]]:
        """
        Sparsify the gradients based on the given weights, server deltas, and beta value.

        Args:
            wts (ScalarWeights_t): Dictionary of weights.
            server_deltas (fT.ActorDeltas_t): Dictionary of server deltas.
        ...
        for cid, zero_del in zeroed_deltas.items():
            if q[cid] != D:
                mask_indices = torch.topk(
                    delta_vec, D - q[cid], largest=False, sorted=False
                ).indices
    ...
    def aggregate(self, strategy_ins: AllClientIns_t) -> CgsvOuts:
        ...
        server_delta, client_deltas = self._update_fn(
        ...
        for cid in client_ids:
            ...
            self._client_wts[cid] = add_momentum_and_clip(
                self._client_wts[cid], cgsv, self.cfg.alpha
            )
            cgsv_vals[cid] = cgsv

        # ic(cgsv_vals)
        self.res_man.log_general_metric(
            cgsv_vals, phase="post_agg", actor="server", metric_name="cgsv"
        )
        ...
        if self.cfg.sparsify_gradients:
            client_deltas, q = self._sparsify_gradients(
                self._client_wts, server_delta, self.cfg.beta
            )
            self.res_man.log_general_metric(
                q, phase="post_agg", actor="server", metric_name="q"
            )
            for cid in client_ids:
                self._client_params[cid] = add_param_deltas(
                    _client_params[cid], client_deltas[cid]
                )
                
                self.res_man.log_parameters(
                    self._client_params[cid], phase="post_agg", actor=cid
                )
        ...
        return CgsvOuts(
            server_params=self._server_params, client_params=self._client_params
        )
