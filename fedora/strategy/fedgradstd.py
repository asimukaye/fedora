...
import torch.optim
# from fedora.customtypes import ClientIns, ClientResult, Result
...
from fedora.strategy.fedavg import random_client_selection, ClientInProto, weighted_parameter_averaging
from fedora.strategy.fedopt import gradient_average_update, add_param_deltas
# from fedora.strategy.fedstdevstrategy import gradient_average_update
...
def get_param_dict_wtd_avg(param_dict: fT.ActorDeltas_t, wts: dict[str, int]) -> float:
...
# def gradient_update_full_dim_weight(server_params: fT.ActorParams_t,client_params: fT.ClientParams_t, weights: ClientTensorWeights_t) -> tuple[fT.ActorParams_t, fT.ActorDeltas_t]:
...
def gradient_update_per_param(server_params: fT.ActorParams_t,client_params: fT.ClientParams_t, weights: ClientScalarWeights_t | ClientTensorWeights_t) -> tuple[fT.ActorParams_t, fT.ActorDeltas_t]:
...
class FedgradIns:
    ...
    client_grad_mus: fT.ActorDeltas_t
    client_grad_stds: fT.ActorDeltas_t
...
class FedgradstdStrategy(ABCStrategy):
    def __init__(self,
        ...
        self._client_params_std: fT.ClientParams_t = {cid: {param: Parameter(torch.empty_like(val.data)) for param, val in self._server_params.items()} for cid in client_ids}
        self._clnt_sigma_by_mu: fT.ClientParams_t = {cid: {param: Parameter(torch.empty_like(val.data)) for param, val in self._server_params.items()} for cid in client_ids}
        # tracking client deltas for logging
        self._client_deltas: fT.ClientDeltas_t = {cid: {param: torch.empty_like(val.data) for param, val in self._server_params.items()} for cid in client_ids}
    ...
    @classmethod
    def client_send_strategy(cls, ins: FedgradIns, result: fT.Result) -> fT.ClientResult:
        ...
        return fT.ClientResult(params=out_params, result=result) 
    ...
    def layer_wise_sbm_log(self, client_ids,
                           ...
                           clnt_sigma_by_mu: dict[str, fT.ActorDeltas_t]):
    ...
    @classmethod
    def _compute_sigma_by_mu_full(cls, sigmas: fT.ActorDeltas_t, mus: fT.ActorDeltas_t) -> fT.ActorDeltas_t:
    ...
    @classmethod
    def _compute_sigma_by_mu_layer_wise(cls, sigmas: fT.ActorDeltas_t, mus: fT.ActorDeltas_t) -> dict[str, float]:
    ...
    @classmethod
    def _compute_sigma_lumped_by_mu_lumped(cls, sigmas: fT.ActorDeltas_t, mus: fT.ActorDeltas_t, param_dims: dict[str, float]) -> float:
