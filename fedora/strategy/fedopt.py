...
from fedora.strategy.fedavg import passthrough_communication, random_client_selection, ClientInProto
import fedora.customtypes as fT

# Type declarations

def add_param_deltas(params: fT.ActorParams_t,
                      deltas: fT.ActorDeltas_t) -> fT.ActorParams_t:
    '''Add deltas to the server parameters'''
    for key, delta in deltas.items():
        params[key].data.add_(delta)
    return params


def compute_server_delta(server_params: fT.ActorParams_t,
                            client_params: fT.ClientParams_t,
                            weights: fT.ScalarWeights_t) -> tuple[fT.ActorDeltas_t, fT.ClientDeltas_t]:
    server_deltas: fT.ActorDeltas_t = {}
    client_deltas: fT.ClientDeltas_t = defaultdict(dict)
...
def gradient_average_update(server_params: fT.ActorParams_t,
                            client_params: fT.ClientParams_t,
                            weights: fT.ScalarWeights_t) -> tuple[fT.ActorParams_t, fT.ActorDeltas_t]:
...
def compute_server_delta_w_normalize(server_params: fT.ActorParams_t,
                            client_params: fT.ClientParams_t,
                            weights: fT.ScalarWeights_t,
                            gamma: float) -> tuple[fT.ActorDeltas_t, fT.ClientDeltas_t]:
    server_deltas: fT.ActorDeltas_t = {}
    client_deltas: fT.ClientDeltas_t = defaultdict(dict)
    for key, server_param in server_params.items():
        for cid, client_param in client_params.items():
...
def gradient_average_with_delta_normalize(server_params: fT.ActorParams_t,
                            client_params: fT.ClientParams_t,
                            weights: fT.ScalarWeights_t,
                            gamma: float) -> tuple[fT.ActorParams_t, fT.ActorDeltas_t]:
...
class FedOptStrategy(ABCStrategy):
    ...
    def send_strategy(self, ids: fT.ClientIds_t) -> fT.ClientIns_t:
        return {cid: fT.ClientIns(params=self._server_params,
    ...
    @classmethod
    def client_send_strategy(cls, ins: FedOptIns, result: fT.Result) -> fT.ClientResult:
