...
class ClientResultStats:
    ...
    results: dict[str, fT.Result] = field(default_factory=dict)
...
class ResultManager:
    ...
    def __init__(self, cfg: SimConfig, logger: Logger) -> None:
        ...
        self.metric_event_actor_dict: dict[str, dict[str, dict[str, dict]]] = (
            defaultdict(lambda: defaultdict(dict))
        )

        # Metric Event Actor Phase dictionary with the phase distinctly listed out at the end
        self.metric_event_actor_phase_dict = defaultdict(
    ...
    def _get_client_results(
        self, result: fT.Results_t, event: str, phase: str
    ) -> ClientResultStats:
        # result_dict = {}
        # client_result = ClientResultStats()
        # client_result.results = result
        # client_result._round = self._round
        client_result = self.compute_client_stats_and_log(
            result, event=event, phase=phase
        )
    ...
    def _round_check(self, rnd, cid):
        ...
        assert (
            self._round == rnd
        ), f"Mismatching rounds: {cid} round: {rnd}, manager round: {self._round}"

    def compute_client_stats_and_log(
        self, client_result: fT.Results_t, event: str, phase: str
    ) -> ClientResultStats:
        ...
        for cid, clnt_result in client_result.items():
            ...
            log_metric(
                f"client:{cid}_{event}", self._round, clnt_result.metrics, self.logger
            )
        ...
        for metric, vals in dict_by_metric.items():
            ...
            log_metric(
                f"{event}_{metric}",
                self._round,
                result_w_stats.stats[metric].__dict__,
                self.logger,
            )
    ...
    def log_general_result(self, result: fT.Result, phase: str, actor: str, event: str):
    ...
    def log_clients_result(
        self, result: fT.Results_t, phase: str, event="local_eval"
    ) -> ClientResultStats:
    ...
    def log_duplicate_parameters_for_clients(
        self,
        cids: t.Union[str, t.List],
        phase: str,
        event: str = "",
        reference_actor="server",
    ):
        ...
        for metric in self.metric_event_actor_dict.keys():
            if "param" in metric.split("/"):
                reference_value = self.metric_event_actor_dict[metric][
                    f"{event}/{phase}"
                ][reference_actor]
    ...
    def log_parameters(
        self,
        model_params: dict[str, Parameter] | dict[str, Tensor],
        phase: str,
        actor: str,
        event: str = "",
        metric="param",
    ...
    def _add_metric(self, metric, event, phase, actor, value):
        ...
        self.metric_event_actor_phase_dict[metric][f"{event}/concat"][actor][
            phase
        ] = value
    ...
    def log_general_metric_average(
        self,
        metrics: dict,
        metric_name: str,
        actor: str,
    ...
    def log_general_metric(
        ...
        if isinstance(metric_val, dict):
            for key, val in metric_val.items():
                self.log_general_metric(
                    val, f"{metric_name}/{key}", actor, phase, event
                )
    ...
    def log_wandb_and_tb(self, result_dict: dict, tag="results"):
        ...
        remove_keys = ["actor", "epoch", "sizes", "size", "round"]
        ...
        scrubbed_dict_2 = self.scrub_dictionary(
            self.metric_event_actor_phase_dict, remove_keys, add_round_back=False
        )

        flattened_dict_2 = pd.json_normalize(
            scrubbed_dict_2, sep="/", max_level=2
        ).to_dict("records")[0]
    ...
    def finalize(self):
        """Save results."""
        self.logger.debug(
            f"[RESULTS] [Round: {self._round:03}] Save results and the global model checkpoint!"
        )
    ...
    def wandb_multi_line_plots(self, plot_keys: list = []):
        ...
        wandb_dict = {}
        in_dict = pd.json_normalize(
            self.metric_event_actor_dict, sep="/", max_level=1
        ).to_dict("records")[0]
        ...
        for plot_key in plot_keys:
            ...
            wandb_dict[plot_key] = wandb_plot.line_series(
                in_dict["round"],
                list(in_dict[plot_key].values()),
                keys=list(in_dict[plot_key].keys()),
                title=plot_key,
                xname="round",
            )
        return wandb_dict
