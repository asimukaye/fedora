...
def custom_pooled(sharded_sets: fT.ClientDatasets_t)->tuple[Dataset, Dataset]:
    train_sets = []
    test_sets = []
...
def fetch_flamby_federated(dataset_name: str, root: str, num_splits: int)->tuple[fT.ClientDatasets_t, Subset]:
    logger.debug(f'[DATA LOAD] Fetching dataset: {dataset_name.upper()}')
